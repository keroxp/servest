...
async function basicRequestPost() {
    ...
    await writeRequest(f, {
        ...
        body: encode("A secure JavaScript/TypeScript runtime built with V8, Rust, and Tokio")
    });
    f.close();
}
async function chunkedRequestPost() {
    ...
    await writeRequest(f, {
        ...
        method: "POST",
        headers,
        body: new StringReader("A secure JavaScript/TypeScript runtime built with V8, Rust, and Tokio")
    });
    f.close();
}
async function chunkedRequestPostWithTrailers() {
    ...
    await writeTrailers(f, headers, new Headers({
        "X-Deno": "land",
        "X-Node": "js"
    }));
    f.close();
